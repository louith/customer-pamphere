...
class IndivWorkerProfile extends StatelessWidget {
  ...
  Future<WorkerDetailsCard?> getWorkerDetailsCard(String userID) async {
    final DocumentSnapshot user =
        await FirebaseFirestore.instance.collection('users').doc(userID).get();

    Map<String, dynamic> userMap = user.data() as Map<String, dynamic>;
    List<String> cats = []; 
    final QuerySnapshotcategories =
        await FirebaseFirestore.instance
            .collection('users')
            .doc(userID)
            .collection('categories')
            .get()
            .then((snapshot) => snapshot.docs.forEach((element) {cats.add(element.id.toString());}));

           
            

     return WorkerDetailsCard(id: userID, name: userMap['name'], role: userMap['role'], address: userMap['address'], categories: categories.toList())       
  }
